...
如果$P ⊆ \mathbb{R} ^2$ 是一个多边形（polygon），那么平面 $\mathbb{R} ^2$ 除去 $P$ 之后恰好有两个区域（regions）。
- 其中一个区域是**无界的**（unbounded），另一个是**有界的**（bounded）。
- 这两个区域都以 P 作为它们的**边界**（frontier）。

这个定理是图论中证明平面图（Planar Graphs）性质的基础，例如它用于欧拉公式（$n−m+f=2$）的证明中，该公式涉及图中区域的数量 $f$。